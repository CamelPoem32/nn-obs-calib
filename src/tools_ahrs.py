...
def plot(*data, **kw):
    ...
    sigmas = kw.get("sigmas")
    if sigmas is not None:
        sigmas = np.full_like(data, sigmas)
    ...
    for i, array in enumerate(data):
        ...
        if array.ndim < 2:
            ...
            if linewidth is not None:
                axs[i, 0].plot(index, array, color=COLORS[0], lw=0.5, ls='-', label=label, linewidth=linewidth)
            else: 
                axs[i, 0].plot(index, array, color=COLORS[0], lw=0.5, ls='-', label=label)
            if sigmas is not None:
                axs[i, 0].fill_between(index, (array - sigmas_i), (array + sigmas_i), color='orange', alpha=.15)
        else:
            ...
            if array_sz[0] > array_sz[1]:
                ...
                if sigmas is not None:
                    sigmas_i = sigmas_i.T
            if indices is not None and indices.shape[0] > indices.shape[1]:
                indices = indices.T

            index = indices[i] if indices is not None else np.arange(array_sz[1])
            # Generate styles
            styles = generate_plot_styles(array_sz[0], markersize=markersize, linewidth=linewidth)
            for j, row in enumerate(array):
                label = None
                if labels:
                    if len(labels[i]) == len(array):
                        label = labels[i][j]
                axs[i, 0].plot(index, row, color=COLORS[j], label=label, lw=0.5, ls='-')           #, **styles[j]
                if sigmas is not None:
                    axs[i, 0].fill_between(index, (row - sigmas_i[j]), (row + sigmas_i[j]), color='orange', alpha=.15)
        axs[i, 0].grid(axis='y')
        if subtitles:
            ...
            try:
                if isinstance(shades_spans, (list, np.ndarray)):
                    ...
                    for k, v in shades_spans.items():
                        ...
                        axs[i, 0].text(np.mean(span), 0.98, k, ha='center', va='top', transform=axs[i, 0].get_xaxis_transform(), clip_on=True,)
            except:
...
# Filter out undesirable markers
good_markers = ['o', 's', 'p', '*', 'H', '+', 'x', 'D', 'd', 'P', 'X']

# Define color palette (contrastive colors)
contrast_colors = [
    '#1f77b4',  # blue
    '#ff7f0e',  # orange
    '#2ca02c',  # green
...
# Generate unique style combinations
def generate_plot_styles(num_styles, markersize=5, linewidth=0.5):
    """Generate unique plot style combinations"""
    styles = []
    color_cycle = cycle(contrast_colors)
    marker_cycle = cycle(good_markers)
    linestyle_cycle = cycle(linestyles)

    if markersize is None: markersize=5
    if linewidth is None: linewidth=0.5
    
    for i in range(num_styles):
        style = {
            # 'color': next(color_cycle),                   # color is already specified in many places
